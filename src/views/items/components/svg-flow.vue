      ...
      <template v-for="(item, index) in nodes">
        <template>
          <g data-name="u1 20">
            <rect
              @click="onClick(index)"
              :width="(item.title.length * 19) + 6 + (item.deletable ? 15 : 0)"
              height="32"
              :x="item.x"
              :y="item.y"
              rx="4"
            ...
            <rect
              ...
              rx="4"
              ry="4"
              :stroke="item.isNow ? 'red' : 'rgb(0,197,212)'"
              stroke-width="1"
              fill="url(#orange_red1)"
              style="cursor: pointer;"
            ></rect>
            <text
              @click="onClick(index)"
              fill="#rgba(36,39,63,1)"
              :y="item.y + parseInt(item.size.height / 2) + 6"
              font-weight="400"
              :x="item.x + parseInt(item.size.width / 2) +
          ...
          <template v-if="!disabled">
            <template v-if="item.deletable">
              <circle
                r="8.6"
                fill="#fbb11b"
                :cx="item.x + item.size.width + 2"
                :cy="item.y + parseInt(item.size.height / 2) + 1"
                stroke="none"
                style="cursor: pointer;"
                @click="onClose(index)"
              ></circle>
              <line
                stroke-width="1.6"
                :x1="item.x + item.size.width + 6"
                :y1="item.y + parseInt(item.size.height / 2) + 5"
                :x2="item.x + item.size.width - 1"
                :y2="item.y + parseInt(item.size.height / 2) - 3"
                stroke="#fff"
                style="cursor: pointer;"
                @click="onClose(index)"
              ></line>
              <line
                stroke-width="1.6"
                :x1="item.x + item.size.width - 1"
                :y1="item.y + parseInt(item.size.height / 2) + 5"
                :x2="item.x + item.size.width + 6"
                :y2="item.y + parseInt(item.size.height / 2) - 3"
                stroke="#fff"
                style="cursor: pointer;"
                @click="onClose(index)"
  ...
  export default {
    ...
    methods: {
      ...
      textSize(text) {
        ...
        result.width = parseFloat(window.getComputedStyle(span).width) - result.width + 14;
        result.height = parseFloat(window.getComputedStyle(span).height) - result.height + 9;
        return result;
      },
