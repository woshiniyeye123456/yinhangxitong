  ...
  <el-card shadow="never" class="default-card">
    ...
    <el-form label-width="120px"
      ...
      <el-form-item label="评审参与人员:">
        <el-select
          v-model="params.testTaskReviewers_ids"
      ...
      <el-form-item label="评审意见:" prop="testTaskReviewComment">
        <el-input
          v-model="params.testTaskReviewComment"
          autosize
          type="textarea"
          :disabled="disabled"
          resize="none"
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        rules: {
          // testTaskReviewers: [{required: true, message: '请输入:评审参与人员', trigger: 'blur'}],
        },
        // 会议纪要
        MMDefaultFileList: [],
        MMFileList: [],
    ...
    methods: {
      ...
      changeTaskReviewers(ids) {
        this.params.testTaskReviewers = [];
        if (ids.length) {
          ids.forEach(id => {
            let team = this.taskReviewersList.find(it => it.id === id);
            if (team) {
              this.params.testTaskReviewers.push(team);
            }
          });
        }
      },
      ...
      save() {
        ...
        params.testTaskReviewMM = this.$common.deepClone(this.MMFileList);
        params.testTaskReviewFiles = this.$common.deepClone(this.filesFileList);
      ...
      setData(data) {
        this.params = data;
        this.$set(this.params, 'testTaskReviewers_ids', this.params.testTaskReviewers_ids || []);
        this.$set(this.params, 'testTaskReviewers', this.params.testTaskReviewers || []);
        // 附件
        if (data.testTaskReviewMM && data.testTaskReviewMM.length) {
          this.MMDefaultFileList = this.$common.deepClone(data.testTaskReviewMM);
          this.MMFileList = this.$common.deepClone(data.testTaskReviewMM);
        }
        if (data.testTaskReviewFiles && data.testTaskReviewFiles.length) {
          this.filesDefaultFileList = this.$common.deepClone(data.testTaskReviewFiles);
          this.filesFileList = this.$common.deepClone(data.testTaskReviewFiles);
        }
      },
