<template>
    <div class="menu-wrapper">
        <template v-for="item in routes" v-if="!item.hidden&&item.children">
            <template v-if="item.access===true">
                <span v-if="hasOneShowingChildren(item.children) && !item.children[0].children&&!item.alwaysShow"
                      @click="toPath('/'+item.children[0].path)"
                ...
                <el-submenu v-else :index="item.name||item.path" :key="item.name">
                    ...
                    <template v-for="child in item.children" v-if="!child.hidden">
                        <template v-if="child.access===true">
                            ...
                            <span v-else @click="toPath(item.path+'/'+ child.path)" :key="child.name">
